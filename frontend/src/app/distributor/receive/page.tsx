...
function DistributorReceiveContent() {
  ...
  if (step === 'done') {
    return (
      <div className="max-w-xl mx-auto text-center py-12 px-4">
        ...
        <h2 className="text-2xl font-bold text-slate-900">Stock Successfully Received!</h2>
        <p className="text-slate-600 mt-2">
          <strong className="text-violet-700 font-semibold">{result?.quantity} units</strong> of{' '}
          <strong>{result?.medicineName}</strong> have been confirmed and added to your distributor inventory.
        </p>
        <p className="text-xs text-slate-400 mt-1">
          Manufacturer has been notified and the batch history is updated on the blockchain ledger.
        </p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Receive Stock from Manufacturer</h1>
          <p className="text-slate-500 text-sm mt-1">
            Scan both QR codes and upload courier proof to confirm receipt and add stock to your distributor inventory.
          </p>
        </div>
        <Link href="/distributor" className="text-sm text-slate-500 hover:text-slate-700 font-medium">
          ← Cancel
        </Link>
      </div>
      ...
      {!isLoadingList && incomingShipments.length > 0 && (
        <div className="bg-violet-50/70 border border-violet-200 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-violet-700 font-medium">Click one to auto-fill</span>
          </div>
                ...
                <div
                  ...
                  className={`p-3.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between ${
                    isSelected
                      ...
                      : 'bg-white/80 border-violet-100 hover:bg-white hover:border-violet-300'
                  }`}
                >
                  <div>
      ...
      {selectedShipment && (
        <div className="p-3 bg-violet-50 border border-violet-200 rounded-xl text-xs text-violet-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span>✅</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-6">
        ...
        <div>
          ...
          {shipmentQr && (
            <div className="mt-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs">
              <span className="text-violet-700 font-medium">✓ Shipment QR captured</span>
        ...
        <div>
          ...
          {medicineQr && (
            <div className="mt-2 p-2.5 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between text-xs">
              <span className="text-violet-700 font-medium">✓ Medicine QR captured</span>
